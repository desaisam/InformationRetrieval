...
if os.path.isdir(pwd + "\\CLEAN_CORPUS"):
    shutil.rmtree((pwd + "\\CLEAN_CORPUS"))
os.makedirs(pwd + "\\CLEAN_CORPUS")
...
for each_file in os.listdir(pwd + "\\RAW_HTML"):
    ...
    if punctuate:
        ...
        output_string.replace('qetuo', '-')
    fo = open(pwd + "\\CLEAN_CORPUS\\" + each_file.split('.html')[0] + ".txt", "a", encoding="utf8")
