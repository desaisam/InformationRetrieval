...
def remove_punctuation(remove_citation_from_content):
    ...
    cleaned_str = "                             "
    transform = str.maketrans(incoming_exception, cleaned_str)
...
def extract_Query(filename_Path, dest_path_query):
    ...
    for k in (sorted(dict_id_queryText.keys())):
...
def main():
    ...
    source_path_raw_copus = input("Please enter the full path where the raw documents are present,\n"
                                      "For which parsing needs to be done \n"
                                      "Example : Z:\Information Retrival\Assignments\Info\Final Project\CACM Corpus \n")
    dest_path_cleaned_corpus = input(
        "Please enter the full path where you would want to store the cleaned corpus,\n"
        "Example : Z:\Information Retrival\Assignments\Info\Final Project\Cleaned CACM Corpus \n")

    # Obtain and Process Queries
    # source_path_query = input("Please enter the full path where the query terms are present,\n"
    #                               "Z:\Information Retrival\Assignments\Info\Final Project\Query_File\Cacm.Query.txt\n")
    # dest_path_query = input("Please enter the full path where the processed query terms to be stored,\n"
    #                             "Z:\Information Retrival\Assignments\Info\Final Project\Query_File\Cacm.Query.txt\n")
