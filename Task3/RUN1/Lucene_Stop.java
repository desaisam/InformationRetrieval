...
public class Lucene_Stop {
	...
	public static void main(String[] args) throws IOException {
		...
		Lucene_Stop indexer = null;
		try {
			indexLocation = s;
			indexer = new Lucene_Stop(s);
		} catch (Exception ex) {
		...
		String filename = "Query_Without_Stop_Words_lu.txt";
		try {
		...
		for (String each : query) {
			try {
				...
				File file = new File("LUCENE_Results.txt");
				for (int i = 0; i < hits.length; ++i) {
					...
					output.write(qId + " Q0 " + f.getName().substring(0, (f.getName().length() - 4)) + " " + (i + 1)
							+ " " + hits[i].score + " LUCENE");
					output.newLine();
					output.close();
				}
	 ...
	 */
	Lucene_Stop(String indexDir) throws IOException {
